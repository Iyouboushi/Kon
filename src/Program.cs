...
#region getConfigValues
            ...
            try
            {
                if (getData(fullpath, "useAnswerLine").ToString() == "true")
                    useAnswerLine = true;
            }
            catch (Exception e)
            {
            ...
            IRC irc;
            irc = new IRC(server, port, mainNick, backupNick, channel, randomTalk, retryAttempts, useAnswerLine, ident, realName);
...
#region generateConfig
        static void generateConfig(String fullpath)
        {
            ...
            try
            {
                ...
                // Do we want the bot using the answers it finds directly without randomizing them?
                configWriter.WriteStartElement("", "useAnswerLine", "");
